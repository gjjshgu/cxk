...
class TreeNode:
    def __init__(self, left_child=None, right_child=None, col=-1, value=None, data=None, result=None):
    ...
    def fit(self, X_train, y_train):
        ...
        def count_info(left, right, data):
            if self.tree_type == 'clf':
                ...
                # base_var = np.var(data[:, -1], ddof=1)
                # left_var = np.var(left[:, -1], ddof=1)
                # right_var = np.var(right[:, -1], ddof=1)
                # frac1 = left.shape[0] / data.shape[0]
                # frac2 = right.shape[0] / data.shape[0]
                # info_gain = base_var - (frac1 * left_var + frac2 * right_var)
                # return info_gain
